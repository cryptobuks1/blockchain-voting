...
describe('Starting tests for voting-angular-app', function() {
  ...
  it('website title should be voting-angular-app', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('voting-angular-app');
    })
  });

  it('network-name should be voting-app-network@0.0.1',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('voting-app-network@0.0.1.bna');
    });
  });

  it('navbar-brand should be voting-angular-app',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('voting-angular-app');
    });
  });
